...
| Apple Shortcuts | Discover the current user's existing Apple Shortcuts through macOS's supported command-line tool, then explicitly enable individual shortcuts or keep selected folders synchronized. Each enabled shortcut becomes a stable MacTools action for Unified Search, global shortcuts, Action Grid, Trackpad Gestures, Automation, and opt-in always-confirmed Run Links. MacTools preserves renamed and temporarily missing items by identifier, provides bounded Run/Stop controls and captured text output, and never reads or modifies private Shortcuts data. |
...
| Auto Input | Remember and restore input sources per app, assign fixed sources, and optionally show the current source beside the focused text field. |
