...
| Window Layouts | Run deterministic commands to arrange the focused window into halves, quarters, thirds, fourths, sixths, edges, a reasonable size, or a custom anchored size; toggle full screen, move between displays, and restore its previous frame. Invoke allowed commands through shortcuts, Run Links, Unified Search, Action Grid, Trackpad Gestures, or workflows—even for MacTools Settings when it is the real frontmost window. A modal shortcut-preset assistant keeps selections pending, separates current and proposed shortcuts, and lets conflicts be re-recorded in place before an explicit Apply. Optional compact feedback confirms successful shortcut and gesture commands, while failures always remain visible. macOS continues to own interactive edge snapping and tiling previews. |
