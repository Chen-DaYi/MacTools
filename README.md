...
| Auto Input | Remember and restore input sources per app, assign fixed sources, switch directly with global shortcuts, and optionally show the current source once whenever an editable field gains focus—including fixed-source apps and MacTools' own Settings and global command searches. The transient HUD also refreshes after input-source changes or app reactivation, appears by the caret when available, and can stay visible while hovered for repeated-click source cycling. |
