...
| Actions & Shortcuts | Discover published MacTools and plugin actions in Unified Search or a central settings page, open the correct owning settings page, run them through one guarded executor, and assign conflict-checked global shortcuts from either surface. Canonical actions cover Sidecar, Window Switcher, display and appearance controls, app/input/audio controls, timed Keep Awake sessions, IP address copying, battery and fan controls, clipboard, disks and Trash, app repair and quitting, guarded Homebrew maintenance, favorite user Launch Items, and physical clean mode. Disk and Xcode cleanup actions only scan and open review; machine-specific, interactive, and destructive actions retain their normal availability, foreground, and confirmation safeguards. Saved Run Links survive relaunch and preferences backup restore. |
| Automation | Build and reorder reusable named workflows with a searchable action picker, run, test, or stop them manually, see activity in Settings and the menu bar, copy detailed per-step diagnostics, and attach schedule, calendar, app, power, display, or network rules with explicit conditions. Definitions and rules are included in preferences backups. |
| Action Grid | Arrange favorite actions, workflows, and nested folders in exact slots of editable 3×3 grids, with one add sheet for actions or folders, drag and drop, in-place settings, keyboard and number-key navigation, multi-display placement, and VoiceOver support. |
...
> **Preferences backup:** Export and import both let you select app preferences, plugin layout, shortcuts, automation, saved Run Links, and each plugin’s portable settings independently, with dependency notes beside every category. Workflow IDs and direct workflow Run Links are preserved; importing an individual workflow preserves its ID on a new Mac and creates a new ID only when that ID already exists. Saved Scripts source is excluded unless each script explicitly opts in, and working directories are never included. Permissions, caches, credentials, and other non-portable data are excluded. Missing plugins are never installed automatically: the preview lets you explicitly select catalog-verified plugins to install, while unavailable or unselected plugins and their settings are skipped.
