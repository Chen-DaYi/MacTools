...
| Plugins & Settings | Install, update, and batch-update plugins in the plugin marketplace (filter by category/search; sort by not-installed first, installed first, or name); enable plugins globally; arrange Dashboard components and Feature Panel actions independently; and manage permissions, shortcuts, and plugin-specific settings. |
