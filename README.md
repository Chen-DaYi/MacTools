...
| Cloudflare R2 Upload | Upload a selected file to an R2 bucket with S3-compatible credentials stored in macOS Keychain, follow byte progress or cancel, and optionally copy its validated public URL from the completion dialog. Object names receive a unique suffix by default to avoid overwrites; preserving the original name is an explicit opt-in. The uploader is also available to global shortcuts, Action Grid, and foreground workflows. |
