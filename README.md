...
| Auto Input | Remember and restore input sources per app, assign fixed sources, switch directly with global shortcuts, and optionally show the current source whenever an editable field gains focus—including fixed-source apps, accessory palettes such as Raycast, and MacTools' own Settings and global command searches. The transient HUD refreshes immediately after input-source changes or app switches, anchors near the caret without covering the full input control, can optionally suppress rapid same-app focus prompts for one second, and can stay visible while hovered for repeated-click source cycling. |
