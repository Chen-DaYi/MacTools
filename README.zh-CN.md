...
> **菜单栏面板：** 仪表盘卡片、嵌套控件与外层弹出面板使用统一的不透明语义背景，并适配浅色、深色和“增强对比度”，避免壁纸影响内容辨识。可在设置中分别选择浅色与深色主题，使用系统默认或十四款内置配色，并实时预览，也可导入 Base16/Base24 YAML/JSON；主题不改变布局、字体、日历事件色和品牌色。
