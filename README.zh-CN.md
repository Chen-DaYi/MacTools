...
| Mac 设置         | 在一个可直接控制的工作区搜索并调整当前 Mac 的 48 项精选设置，包括即时生效的指针大小、缩放手势、程序坞、访达、外观、原彩显示和夜览控制；保存指针大小需要一次性授予完全磁盘访问。把有序收藏放进功能面板，查看最近更改和可处理的问题，并通过比较、逐项选择、验证、结构化结果与回滚来创建和应用可移植的类型化配置。尚未验证的辅助功能、输入、电源和网络设置会诚实地引导到系统设置。 |
