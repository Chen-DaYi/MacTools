...
| Cloudflare R2 上传 | 使用保存在 macOS 钥匙串中的 S3 兼容凭据上传文件；上传前可重命名，或生成保留扩展名的随机 UUID 文件名，遇到同名对象时可选择取消、重新命名或覆盖，并在同一窗口查看进度或取消。上传完成后可在弹窗中复制校验通过的公开链接；支持全局快捷键、Action Grid 和前台工作流。 |
