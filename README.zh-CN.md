...
| 操作与快捷键     | 在统一搜索或集中设置页发现并运行 MacTools 与插件发布的操作，并通过同一套冲突检测与注册服务分配全局快捷键。                  |
...
| 插件与设置       | 在任一 MacTools 界面按 ⌘K、点击插件侧边栏的搜索入口，或为「打开命令面板」配置可选全局快捷键，即可打开统一搜索面板，查找应用页面、通用设置、插件、具体插件设置项和明确支持的命令；全局快捷键默认不分配，配置后可从任意应用打开独立的键盘优先面板。命令可明确选择跟随系统、深色或浅色外观，开启或关闭登录时启动，并按插件支持情况在仪表盘或功能面板中显示或隐藏插件；只提供会改变当前状态的操作。可用 ↑/↓ 或 Tab/Shift-Tab 选择结果，Return 运行或打开，Command-Return 打开该操作准确对应的设置/编辑页，也可按 ⌘1–⌘9 快速打开前九项可见结果。⌘F 继续用于插件市场等当前页面的局部搜索。通过菜单栏面板顶部居中的切换器选择功能或组件面板；在插件市场中安装、更新和批量更新插件，并在各插件设置页维护权限、快捷键和专属设置；导出和导入时可分别选择应用偏好、插件布局、快捷键、自动化、已保存 Run Link 及各插件的可移植设置。 |
