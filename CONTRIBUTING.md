...
- Public Nightly releases are maintainer-operated snapshots of `main`. They use a separate app identity, storage scope, update feed, and complete same-commit plugin catalog. Nightly plugin versions are generated as `source-major.run.attempt`; feature contributions must not bump source manifests for Nightly. After two successful manual update runs, maintainers may enable the daily schedule with `ENABLE_NIGHTLY_RELEASES=true`.
