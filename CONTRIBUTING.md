...
- Plugin library releases are triggered by `plugins-*` batch tags through the `Plugin Release` workflow. Within one PluginKit ABI line, plugins with bumped versions are built and uploaded, then merged into that line's catalog. Changes under `Sources/MacToolsPluginKit/` require rebuilding and bumping every plugin so the catalog cannot retain binaries linked against an older shared framework. The first release of a new ABI also rebuilds every plugin and writes a versioned catalog. PluginKit v4 hosts before 1.2 keep reading the immutable `docs/plugins/v4/catalog.json`; MacTools 1.2 and later read `docs/plugins/v4/host-1.2/catalog.json`, whose verifier supports mixed per-plugin host requirements. Publish the host-1.2 plugin catalog before the 1.2 app so upgraded hosts can immediately retrieve rebuilt packages without disrupting 1.1.6. The catalog private key, Developer ID certificate, and GitHub token must come from CI secrets or local environment variables.
