...
- When a plugin adopts an action-surface or other newly exported PluginKit type, set its `plugin.json.minHostVersion` to the first compatible app release. Release tooling owns package and app version bumps; compatibility metadata belongs in the source manifest.
