...
- A plugin that composes an existing provider's canonical action may adopt `PluginActionExecutionHostContextConsuming`; do not call the provider implementation or duplicate its system write directly. Keep the bridge limited to lookup and execution so host availability, safety, confirmation, and diagnostics remain authoritative. Mac Settings is the reference implementation in `docs/plugins/mac-settings.md`.
